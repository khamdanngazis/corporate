...
func (c DbConnection) Open() (*sql.DB, error) {
	...
	//fullUrl combine username and password with dburl
	// var DBUrl string
	// //DBUrl = conf.Param.DBUserName + ":" + conf.Param.DBPassword + "@tcp(" + conf.Param.DBServer + ")/" + conf.Param.DBName
	// log.Debugf("Initiating database connection - %s", c.URL)
	// dbConn, err := sql.Open(c.Type, c.URL)

	dbConn, err := sql.Open(conf.Param.DBType, conf.Param.DBUrl)
	if err != nil {
		return nil, err
	}
	...
	log.Logf("Initiating database connection %s", conf.Param.DBUrl)
	return dbConn, nil
}
...
// InsertGetLastID is use for ...
func (c DbConnection) Insert(sqlStringName string, args ...interface{}) (int64, error) {
	// if no dbConnection, return
	//
	if c.Db == nil {
		return 0, fmt.Errorf("Please OpenConnection prior Query")
